...
VLAN是一个在物理网络上根据用途，工作组、应用等来逻辑划分的局域
网络，与用户的物理位置没有关系。
...
1. 帧控制：具有多种用途 
2. 持续时间：下一个要发送帧可能持续的时间（NAV）或关联ID（AID） 
3. 地址1~地址4：每个地址的含义基于“去往DS”和“来自DS”域段确定    
4. 顺序控制：过滤掉重复帧，或用于分片组合   
5. QoS控制域段：存放数据流的QoS信息（802.11e中扩展）    
6. 数据：包含任意长度的数据（0-2312字节）   
7. CRC校验：802.11采用4个字节的校验码   
